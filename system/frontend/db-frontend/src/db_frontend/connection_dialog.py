...
class ConnectionDialog(tk.Toplevel):
    def __init__(self, parent: tk.Tk):
        ...
        tk.Entry(self, textvariable=self._key,
                 width=38, show="*").grid(row=1, column=1, padx=10)

        tk.Button(self, text="Verbinden",
    ...
    def _confirm(self):
        if not self._url.get().startswith("http"):
            messagebox.showwarning("Eingabe", "Bitte gueltige URL eingeben.")
            return
        self.confirmed = True
        self.destroy()
